...
export class FlwSwitchComponent implements ControlValueAccessor {@Input() label='';value=false;disabled=false;private change=(v:boolean)=>{};touched=()=>{};writeValue(v:boolean){this.value=!!v}registerOnChange(fn:(v:boolean)=>void){this.change=fn}registerOnTouched(fn:()=>void){this.touched=fn}setDisabledState(v:boolean){this.disabled=v}set(v:boolean){this.value=v;this.change(v)}}
