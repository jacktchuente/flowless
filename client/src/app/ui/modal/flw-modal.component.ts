...
@Component({
  ...
  template: ` <section
    ...
    role="dialog"
    aria-modal="true"
    [attr.aria-labelledby]="title ? 'flw-modal-title' : null"
  >
    <header class="modal-head">
      <div>
        <h3 id="flw-modal-title">{{ title || data?.title }}</h3>
        <p class="desc" *ngIf="description || data?.description">
...
export class FlwModalComponent {
  ...
  @Input() wide = false;
  constructor(
    @Optional() public ref: DialogRef<unknown>,
    @Optional() @Inject(DIALOG_DATA) public data: FlwModalData | null,
  ) {}
  close() {
    this.ref?.close();
  }
