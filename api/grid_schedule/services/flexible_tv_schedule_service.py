...
FlexibleGenerationResult = PlayoutGenerationResult


class FlexibleTvPlayoutGenerationService(BasePlayoutGenerationService):
    ...
    def __init__(self, *, tv_channel: TvChannel, days: int, reset: bool = False, extend: bool = False):
        super().__init__(tv_channel=tv_channel, days=days, reset=reset, extend=extend)
    ...
    def _resolve_post_roll_filler_end(self, *, item_end: datetime, window_end: datetime) -> datetime | None:
        ...
        return self._resolve_post_roll_filler_end_for_policy(
            policy=grid.post_filler_policy if grid.post_filler_policy_id else None,
